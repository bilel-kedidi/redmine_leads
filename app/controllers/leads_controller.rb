...
class LeadsController < ApplicationController
    ...
    respond_to do |format|
      format.js{
        ...
        if @type.include?('phone')
          ...
          @exist = Contact.where(skype_name: @value).present?
        elsif @type.include?('email')
          @exist = Contact.where(email: @value).present?
        end
      }
    end
  end
  ...
  def context_menu
    ...
    @can = {:edit => (@contact && @contact.editable?) || (@contacts && @contacts.collect{|c| c.editable?}.inject{|memo,d| memo && d}),
            ...
            :create => (@project && (User.current.allowed_to?(:add_contacts, @project) or User.current.allowed_to?(:add_leads, @project))),
