...
class Lead < ActiveRecord::Base
  ...
  acts_as_watchable
  acts_as_attachable :view_permission =>  :view_leads,
                     :delete_permission => :edit_leads


  acts_as_event :datetime => :created_on,
                :url => lambda {|o| {:controller => 'leads', :action => 'show', :id => o}},
                :type => 'icon-contact',
  ...
  scope :visible, lambda {|*args| eager_load(:projects).where(Lead.visible_condition(args.shift || User.current, *args)) }
  scope :deletable, lambda {|*args| eager_load(:projects).where(Lead.deletable_condition(args.shift || User.current, *args)).readonly(false) }
  scope :editable, lambda {|*args| eager_load(:projects).where(Lead.editable_condition(args.shift || User.current, *args)).readonly(false) }
  scope :by_project, lambda {|prj| joins(:projects).where("#{Project.table_name}.id = ?", prj) unless prj.blank? }
  scope :like_by, lambda {|field, search| {:conditions => ["LOWER(#{Lead.table_name}.#{field}) LIKE ?", search.downcase + "%"] }}
  scope :companies, lambda { where(:is_company => true) }
  scope :people, lambda { where(:is_company => false) }
  scope :order_by_name, lambda { order(Lead.fields_for_order_statement) }
  scope :order_by_creation, lambda { order("#{Lead.table_name}.created_on DESC") }

  scope :by_full_name, lambda {|search| where("LOWER(CONCAT(#{Lead.table_name}.first_name,' ',#{Lead.table_name}.last_name)) = ? ", search.downcase)}
  scope :by_name, lambda {|search| where("(LOWER(#{Lead.table_name}.first_name) LIKE LOWER(:p) OR
                                                                  LOWER(#{Lead.table_name}.last_name) LIKE LOWER(:p) OR
                                                                  LOWER(#{Lead.table_name}.middle_name) LIKE LOWER(:p))",
                                                                  {:p => "%" + search.downcase + "%"})}

  scope :live_search, lambda {|search| where("(LOWER(#{Lead.table_name}.first_name) LIKE LOWER(:p) OR
                                               LOWER(#{Lead.table_name}.last_name) LIKE LOWER(:p) OR
                                               LOWER(#{Lead.table_name}.middle_name) LIKE LOWER(:p) OR
                                               LOWER(#{Lead.table_name}.company) LIKE LOWER(:p) OR
                                               LOWER(#{Lead.table_name}.email) LIKE LOWER(:p) OR
                                               LOWER(#{Lead.table_name}.phone) LIKE LOWER(:p) OR
                                               LOWER(#{Lead.table_name}.job_title) LIKE LOWER(:p))",
                                             {:p => "%" + search.downcase + "%"})}
  ...
  def self.visible_condition(user, options={})
    ...
    projects_allowed_to_view_contacts = Project.where(Project.allowed_to_condition(user, :view_leads)).pluck(:id)
    ...
    if user.admin?
      ...
      cond << " (#{table_name}.visibility = 1) OR" if user.allowed_to_globally?(:view_leads, {})
  ...
  def self.available_tags(options = {})
    ...
    scope = scope.where(Lead.visible_condition(options[:user] || User.current))
    ...
    joins << "JOIN #{Lead.table_name} ON #{Lead.table_name}.id = #{RedmineCrm::Tagging.table_name}.taggable_id AND #{RedmineCrm::Tagging.table_name}.taggable_type =  '#{Lead.name}' "
    joins << Lead.projects_joins
  ...
  def duplicates(limit=10)
    ...
    scope = scope.where("#{Lead.table_name}.id <> ?", self.id) if !self.new_record?
    @duplicates ||= (self.first_name.blank? && self.last_name.blank? && self.middle_name.blank?) ? [] : scope.visible.limit(limit)
  end

  def company_contacts
    @contacts ||= Lead.order_by_name.includes(:avatar).where(["#{Lead.table_name}.is_company = ?  AND #{Lead.table_name}.company = ? AND #{Lead.table_name}.id <> ?", false, self.first_name, self.id])
  end
  ...
  def contact_company
    @contact_company ||= Lead.where(:first_name => self.company, :is_company => true).
      where("#{Lead.table_name}.id <> #{self.id}").first unless self.company.blank?
  end
  ...
  def visible?(usr=nil)
    usr ||= User.current
    if self.is_public?
      usr.allowed_to_globally?(:view_leads, {})
    else
      self.allowed_to?(usr || User.current, :view_leads)
    end
  end
  ...
  def project(current_project=nil)
    ...
    if current_project && self.projects.visible.include?(current_project)
      ...
      @project  = self.projects.visible.where(Project.allowed_to_condition(User.current, :view_leads)).first
    end
  ...
  def self.find_by_emails(emails)
    ...
    emails.each do |mail|
      cond << " OR (LOWER(#{Lead.table_name}.email) LIKE LOWER('%#{mail.gsub("'", "").gsub("\"", "")}%'))"
    end
    contacts = Lead.where(cond)
  ...
  # Examples:
  #
  #   Lead.fields_for_order_statement              => ['contacts.first_name', 'contacts.first_name', 'contacts.id']
  #   Lead.fields_for_order_statement('customers')   => ['customers.last_name', 'customers.id']
