...
class LeadQuery < CrmQuery

  self.queried_class = Lead

  self.available_columns = [
    QueryColumn.new(:id, :sortable => "#{Lead.table_name}.id", :default_order => 'desc', :caption => '#'),
    QueryColumn.new(:name, :sortable => lambda {Lead.fields_for_order_statement}, :caption => :field_contact_full_name),
    QueryColumn.new(:first_name, :sortable => "#{Lead.table_name}.first_name"),
    QueryColumn.new(:last_name, :sortable => "#{Lead.table_name}.last_name"),
    QueryColumn.new(:middle_name, :sortable => "#{Lead.table_name}.middle_name", :caption => :field_contact_middle_name),
    QueryColumn.new(:job_title, :sortable => "#{Lead.table_name}.job_title", :caption => :field_contact_job_title, :groupable => true),
    QueryColumn.new(:company, :sortable => "#{Lead.table_name}.company", :groupable => "#{Lead.table_name}.company", :caption => :field_contact_company),
    QueryColumn.new(:phones, :sortable => "#{Lead.table_name}.phone", :caption => :field_contact_phone),
    QueryColumn.new(:emails, :sortable => "#{Lead.table_name}.email", :caption => :field_contact_email),
    ...
    QueryColumn.new(:created_on, :sortable => "#{Lead.table_name}.created_on"),
    QueryColumn.new(:updated_on, :sortable => "#{Lead.table_name}.updated_on"),
  ...
  def initialize_available_filters
    ...
    add_available_filter "tags", :type => :list, :values => Lead.available_tags(project.blank? ? {} : {:project => project.id}).collect{ |t| [t.name, t.name] }, :order => 12
  ...
  def sql_for_tags_field(field, operator, value)
    ...
    ids_list  = Lead.tagged_with(value).collect{|contact| contact.id }.push(0).join(',')
    "( #{Lead.table_name}.id #{compare} (#{ids_list}) ) "
  end
  ...
  def sql_for_has_open_issues_field(field, operator, value)
    ...
    if operator == "!*"
      "#{Lead.table_name}.id IN (
        SELECT #{Lead.table_name}.id FROM #{Lead.table_name}
        LEFT JOIN contacts_issues ON contacts_issues.contact_id = #{Lead.table_name}.id
  ...
  def sql_for_last_note_field(field, operator, value)
    ...
    if operator == "!*"
      "#{Lead.table_name}.id IN (
        SELECT #{Lead.table_name}.id FROM #{Lead.table_name}
        LEFT JOIN #{db_table} ON #{db_table}.source_id = #{Lead.table_name}.id and #{db_table}.source_type = 'Contact'
        GROUP BY #{Lead.table_name}.id
        HAVING COUNT(#{db_table}.id) = 0)"
    elsif operator == "*"
      "#{Lead.table_name}.id IN (
        SELECT #{Lead.table_name}.id FROM #{Lead.table_name}
        INNER JOIN #{db_table} ON #{db_table}.source_id = #{Lead.table_name}.id and #{db_table}.source_type = 'Contact'
        GROUP BY #{Lead.table_name}.id
        HAVING COUNT(#{db_table}.id) > 0)"
    else
      "#{Lead.table_name}.id IN (
